...
async function readRootPath() {
    return "/Users/coffeecup/Documents/open_source/kubernetes/prometheus/discovery/manager.go"
...
async function readRootFunction() {
    return "func NewManager(ctx context.Context, logger *slog.Logger, registerer prometheus.Registerer, sdMetrics map[string]DiscovererMetrics, options ...func(*Manager)) *Manager {"
    const rl = readline.createInterface({input, output})
...
async function readPurpose() {
    return "prometheusのdiscovery Managerが各メトリクスを取得する部分が知りたい"
    const rl = readline.createInterface({input, output})
